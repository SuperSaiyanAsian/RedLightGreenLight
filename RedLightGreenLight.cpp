...
// Mugunghwa Kkoci Pieot Seumnida == The hibiscus flower has bloomed.
...
//
//	Author:			Lance Adriano

// title of these windows:

const char* WINDOWTITLE = { "CS 450 Final Project - Lance Adriano" };
...
// which texture display:

enum ObjectTextureDisplay
{
	OFF,
	ON,
	DISTORTED
...
bool    Frozen;					// boolean to determine whether to freeze animation
int		MainWindow;				// window id for main graphics window
float	Scale;					// scaling factor
float	Time;					// timer in the range [0.,1.)
int		WhichColor;				// index into Colors[ ]
int		WhichProjection;		// ORTHO or PERSP
int		WhichTextureDisplay;	// OFF, ON, or DISTORTED
...
const float FLOOR = { -18. };
const float CEILING = { 100. };
const float LEFT_WALL = { -100. };
const float RIGHT_WALL = { 100. };
const float FRONT_WALL = { -700. };
const float BACK_WALL = { 200. };
const float FLOOR_COLOR[3] = { .5f,.5f,.5f };
const float LEFT_WALL_COLOR[3] = { .2f,.2f,.2f };
const float FRONT_WALL_COLOR[3] = { .3f,.3f,.3f };
...
OsuSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
											...
											// ilng=NumLngs-1, lng=+M_PI are the same meridian
		{
			...
			if (WhichTextureDisplay == DISTORTED)
			{
				p->s = (cosf(lng) + M_PI + Time) / (1.5 * M_PI / Time);
				p->t = (sinf(lat) + M_PI - Time / 1.5 + Time) / M_PI * Time;
			}
...
GLfloat bodyColor[] = { 0.9, 0.4, 0.1 };
GLfloat shouldersColor[] = { 1., 1., 0.2 };
GLfloat headarmsColor[] = { 1., 0.9, 0.7 };
GLfloat hairshoesColor[] = { 0, 0, 0 };
GLfloat eyeslegsColor[] = { 1., 1., 1. };
GLfloat beadsColor[] = { .8, .5, .7 };
GLfloat treeColor[] = { .5, .3, .1 };

GLuint Body;				// Orange 9 4 1
GLuint LeftShoulder;		// Yellow 1. 1. .2
GLuint RightShoulder;		// Yellow 1. 1. .2
GLuint Head;				// Skin 1. 9 7
GLuint LeftArm;				// Skin 1. 9 7
GLuint RightArm;			// Skin 1. 9 7
GLuint Hair;				// Black 0 0 0
GLuint Shoes;				// Black 0 0 0
GLuint Eyes;				// White 1. 1. 1.
GLuint Legs;				// White 1. 1. 1.
GLuint Beads;				// Purple 8 5 7
...
void
RedLightGreenLight(int state);

void
RedLightGreenLightRemix(int state);
...
void Camera()
{
	...
	if (motion.Forward)
	{
		camX += cos((yaw + 90) * D2R) / 30.;
		camZ -= sin((yaw + 90) * D2R) / 30.;
	}
	if (motion.Backward)
	{
		camX += cos((yaw + 90 + 180) * D2R) / 30.;
		camZ -= sin((yaw + 90 + 180) * D2R) / 30.;
	}
	if (motion.Left)
	{
		camX += cos((yaw + 90 + 90) * D2R) / 30.;
		camZ -= sin((yaw + 90 + 90) * D2R) / 30.;
	}
	if (motion.Right)
	{
		camX += cos((yaw + 90 - 90) * D2R) / 30.;
...
main(int argc, char* argv[])
{
	...
	//PlaySound("RedLightGreenLightRemix.wav", NULL, SND_ASYNC | SND_FILENAME);
...
Display()
{
	...
	Pattern->SetUniformVariable("uSpecularColor", White[0], White[1], White[2]);
	Pattern->SetUniformVariable("uShininess", shine);
	Pattern->SetUniformVariable("uKa", ambient);
	Pattern->SetUniformVariable("uKd", diffuse);
	Pattern->SetUniformVariable("uKs", specular);
	Pattern->SetUniformVariable("uTexOn", TexOn);

	Pattern->SetUniformVariable("uTexUnit", 2); // tell your shader program which texture unit to choose
	glCallList(WallList);


	Pattern->SetUniformVariable("uTexUnit", 3); // tell your shader program which texture unit to choose
	glCallList(BackWallList);
	...
	Pattern->SetUniformVariable("uColor", bodyColor[0], bodyColor[1], bodyColor[2]);
	glCallList(Body);

	Pattern->SetUniformVariable("uColor", shouldersColor[0], shouldersColor[1], shouldersColor[2]);
	...
	if (motion.Forward || motion.Backward || motion.Left || motion.Right)
	{
		if (camZ <= -600)
		{
			...
			if (survived == 1)
			{
				glutTimerFunc(0.1, RedLightGreenLightRemix, 0);
			}
		}
	}

	if (nowTime > 7. && invincible == false)
	{
		if (motion.Forward || motion.Backward || motion.Left || motion.Right)
		{
			motion.Forward = false;
	...
	Pattern->SetUniformVariable("uColor", headarmsColor[0], headarmsColor[1], headarmsColor[2]);
	glCallList(Head);

	Pattern->SetUniformVariable("uColor", hairshoesColor[0], hairshoesColor[1], hairshoesColor[2]);
	glCallList(Hair);

	Pattern->SetUniformVariable("uColor", beadsColor[0], beadsColor[1], beadsColor[2]);
	glCallList(Beads);

	glPushMatrix();
	Pattern->SetUniformVariable("uColor", eyeslegsColor[0], eyeslegsColor[1], eyeslegsColor[2]);
	glScalef(0.95, 1, 1);
	glCallList(Eyes);
	glPopMatrix();

	glPushMatrix();
	Pattern->SetUniformVariable("uColor", 0., 0., 0.);
	glTranslatef(-0.75, 2.95, 0.945);
	...
	TexOn = TRUE;
	Pattern->SetUniformVariable("uTexOn", TexOn);
	Pattern->SetUniformVariable("uTexUnit", 1); // tell your shader program which texture unit to choose
	glPushMatrix();
	glScalef(5, 6.5, 5);
	glTranslatef(0, -2.75, -1.75);
	glRotatef(-120, 0, 1, 0);
	glCallList(Tree);
	glPopMatrix();
	...
	glDisable(GL_DEPTH_TEST);
	glColor3f(0., 1., 1.);
	//DoRasterString( 0., 1., 0., (char *)"Check Out My Project 2!" );
	...
	if (camZ <= -600)
	{
		DoRasterString(42., 70., 0., (char*)"You Survived!");
	}
...
InitGraphics()
{
	...
	glutTimerFunc(10000, RedLightGreenLight, 0);
	glutIdleFunc(Animate);

	// Read in texture from bmp file

	//int width, height;

	//unsigned char* Texture = BmpToTexture("The Great Wave.bmp", &width, &height);
	...
	// Tree
	Texture = BmpToTexture("TreeTexture.bmp", &width, &height);
	glBindTexture(GL_TEXTURE_2D, TreeTex);
	...
	Texture2 = BmpToTexture("Wallpaper.bmp", &width2, &height2);
	glBindTexture(GL_TEXTURE_2D, Walls);
	...
	// Back Wall
	glGenTextures(1, &BackWall);
	int width3, height3;

	Texture3 = BmpToTexture("People.bmp", &width3, &height3);
	glBindTexture(GL_TEXTURE_2D, BackWall);
	...
	// X Positions
	Xpos.AddTimeValue(0., -0.5);
	Xpos.AddTimeValue(1., -0.3);
	Xpos.AddTimeValue(2., -0.);
	Xpos.AddTimeValue(3., -0.);
	Xpos.AddTimeValue(4., -0.);
	Xpos.AddTimeValue(5., -0.);
	Xpos.AddTimeValue(6., -0.);
	Xpos.AddTimeValue(7., -0.5);
	Xpos.AddTimeValue(8., -0.5);
	Xpos.AddTimeValue(9., -0.5);
	Xpos.AddTimeValue(10., -0.5);

	// Y Positions
	Ypos.AddTimeValue(0.f, 0.);
	Ypos.AddTimeValue(1.337, 0.);
	Ypos.AddTimeValue(2., -0.);
	Ypos.AddTimeValue(2.6, 0.);
	Ypos.AddTimeValue(3.3, 0.);
	Ypos.AddTimeValue(4.2, 0.);
	Ypos.AddTimeValue(5., 0.);
	Ypos.AddTimeValue(5.67, 0.);
	Ypos.AddTimeValue(6.9, 0.);
	Ypos.AddTimeValue(7.7, 0.);
	Ypos.AddTimeValue(8.4, 0.);
	Ypos.AddTimeValue(9.6, 0.);
	Ypos.AddTimeValue(10.f, 0.);

	// Z Positions
	Zpos.AddTimeValue(0.f, 0.);
	Zpos.AddTimeValue(1.337, 0.);
	Zpos.AddTimeValue(2., 0.);
	Zpos.AddTimeValue(2.6, -0.);
	Zpos.AddTimeValue(3.3, 0.);
	Zpos.AddTimeValue(4.2, 0.);
	Zpos.AddTimeValue(5., 0.);
	Zpos.AddTimeValue(5.67, -0.);
	Zpos.AddTimeValue(6.9, 0.);
	Zpos.AddTimeValue(7.7, 0.);
	Zpos.AddTimeValue(8.4, 0.);
	Zpos.AddTimeValue(9.6, -0.);
	Zpos.AddTimeValue(10.f, 0.);

	// Theta X
	ThetaX.AddTimeValue(0., 0.);
	ThetaX.AddTimeValue(1., 0.);
	ThetaX.AddTimeValue(2., 0.);
	ThetaX.AddTimeValue(3., 0.);
	ThetaX.AddTimeValue(4., 0.);
	ThetaX.AddTimeValue(5., 0.);
	ThetaX.AddTimeValue(6., 0.);
	ThetaX.AddTimeValue(7., 0.);
	ThetaX.AddTimeValue(8., 0.);
	ThetaX.AddTimeValue(9., 0.);
	ThetaX.AddTimeValue(10., 0.);

	// Theta Y
	ThetaY.AddTimeValue(0., 180.);
	ThetaY.AddTimeValue(1., 80.);
	ThetaY.AddTimeValue(2., 0.);
	ThetaY.AddTimeValue(3., 0.);
	ThetaY.AddTimeValue(4., 0.);
	ThetaY.AddTimeValue(5., 0.);
	ThetaY.AddTimeValue(6., 0.);
	ThetaY.AddTimeValue(7., 180.);
	ThetaY.AddTimeValue(8., 180.);
	ThetaY.AddTimeValue(9., 180.);
	ThetaY.AddTimeValue(10., 180.);

	// Theta Z
	ThetaZ.AddTimeValue(0.f, 0.);
	ThetaZ.AddTimeValue(1.337, 0.);
	ThetaZ.AddTimeValue(2., 0.);
	ThetaZ.AddTimeValue(2.6, 0.);
	ThetaZ.AddTimeValue(3.3, 0.);
	ThetaZ.AddTimeValue(4.2, 0.);
	ThetaZ.AddTimeValue(5., 0.);
	ThetaZ.AddTimeValue(5.67, 0.);
	ThetaZ.AddTimeValue(6.9, 0.);
	ThetaZ.AddTimeValue(7.7, 0.);
	ThetaZ.AddTimeValue(8.4, 0.);
	ThetaZ.AddTimeValue(9.6, 0.);
	ThetaZ.AddTimeValue(10.f, 0.);
...
InitLists()
{
	...
	LoadObjFile("Body.obj");
	glEndList();

	LeftShoulder = glGenLists(1);
	glNewList(LeftShoulder, GL_COMPILE);
	LoadObjFile("Shoulder_Left.obj");
	glEndList();

	RightShoulder = glGenLists(1);
	glNewList(RightShoulder, GL_COMPILE);
	LoadObjFile("Shoulder_Right.obj");
	glEndList();

	Head = glGenLists(1);
	glNewList(Head, GL_COMPILE);
	LoadObjFile("Head.obj");
	glEndList();

	LeftArm = glGenLists(1);
	glNewList(LeftArm, GL_COMPILE);
	LoadObjFile("Arm_Left.obj");
	glEndList();

	RightArm = glGenLists(1);
	glNewList(RightArm, GL_COMPILE);
	LoadObjFile("Arm_Right.obj");
	glEndList();

	Hair = glGenLists(1);
	glNewList(Hair, GL_COMPILE);
	LoadObjFile("Hair.obj");
	glEndList();

	Shoes = glGenLists(1);
	glNewList(Shoes, GL_COMPILE);
	LoadObjFile("Shoes.obj");
	glEndList();

	Eyes = glGenLists(1);
	glNewList(Eyes, GL_COMPILE);
	LoadObjFile("Eyes.obj");
	glEndList();

	Legs = glGenLists(1);
	glNewList(Legs, GL_COMPILE);
	LoadObjFile("Legs.obj");
	glEndList();

	Beads = glGenLists(1);
	glNewList(Beads, GL_COMPILE);
	LoadObjFile("Beads.obj");
	glEndList();
	...
	glTexCoord2f(1, 0);
	glVertex3f(RIGHT_WALL, CEILING, FRONT_WALL);

	glTexCoord2f(1, 1);
	glVertex3f(RIGHT_WALL, CEILING, BACK_WALL);
	...
	BackWallList = glGenLists(1);
	glNewList(BackWallList, GL_COMPILE);
	glColor3fv(FLOOR_COLOR);
	glBegin(GL_QUADS);
...
Reset()
{
	...
	//WhichProjection = ORTHO;
	WhichProjection = PERSP;
	WhichTextureDisplay = ON;
	Xrot = Yrot = 0.;
	pitch = yaw = camX = camZ = 0;
}
...
RedLightGreenLight(int state)
{
	PlaySound("HibiscusHasBloomed.wav", NULL, SND_ASYNC | SND_FILENAME | SND_NOSTOP);
	glutTimerFunc(10000, RedLightGreenLight, 0);
}
...
RedLightGreenLightRemix(int state)
{
	...
	PlaySound("RedLightGreenLightRemix.wav", NULL, SND_ASYNC | SND_FILENAME | SND_LOOP);
}
...
{
	PlaySound(NULL, NULL, SND_NODEFAULT);
	PlaySound("Gunshot.wav", NULL, SND_ASYNC | SND_FILENAME);
	Reset();
}
